...
class Test(unittest.TestCase):
    ...
    def test_get_result_manager(self):
        ...
        fig_list = rm.get_plot_all(order=order, x_metric='epoch', y_metric_list=['acc', 'epoch'], title_list=['dataset'], 
                              legend_list=['model'], avg_across='model')
        for i, fig in enumerate(fig_list):
            fig.savefig(os.path.join(savedir_base, '%s_%d.png' % (order, i)))
    ...
    def test_get_job_stats_logs_errors(self):
        ...
        try:
            ...
            stats = hr.get_job_stats(exp_list, savedir_base=savedir_base, username='issam')
            logs = hr.get_job_logs(exp_list, savedir_base=savedir_base, username='issam')
            errors = hr.get_job_errors(exp_list, savedir_base=savedir_base, username='issam')
        except:
            pass
